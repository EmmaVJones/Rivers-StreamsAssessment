...
conventionalsWithAU_WQS <- read_csv('data/final2020data/CEDSWQM_2020_IR_DATA-CONVENTIONALS_20190305.csv') %>%
  filter(!is.na(Latitude)|!is.na(Longitude)) %>% # remove sites without coordinates 
  left_join(dplyr::select(conventionals_D_Region_EstuaryAU_WQS, FDT_STA_ID, ID305B_1:Shape_Area),
            by='FDT_STA_ID') %>%
...
write.csv(conventionalsWithAU_WQS2, 'data/TRO/conventionals_Estuary_AU_WQS.csv',row.names = F)
```
