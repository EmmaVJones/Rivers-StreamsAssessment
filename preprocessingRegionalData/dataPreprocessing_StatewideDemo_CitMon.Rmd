...
nonARawFinal <- left_join(nonARaw3, 
                     select(nonA, rowname, FDT_STA_ID, STA_DESC, Latitude, Longitude), by='rowname') %>%
  mutate(FDT_STA_ID = FDT_STA_ID.y, STA_DESC = STA_DESC.y, Latitude = Latitude.y, Longitude = Longitude.y) %>% # keep what mary/paula called
  select(-c(FDT_STA_ID.x, STA_DESC.x, Latitude.x, Longitude.x, FDT_STA_ID.y, STA_DESC.y, Latitude.y, Longitude.y)) %>%
...
RegionalResultsCombined2 <- mutate(RegionalResultsCombined, VAHU6=VAHU6.x) %>%
  dplyr::select( names(template))
...
RegionalResults4 $RorL <- NA
for(i in 1: nrow(RegionalResults4)){
  RegionalResults4 $RorL[i] <- substr(strsplit(as.character(RegionalResults4 $ID305B_1[i]), '-')[[1]][2] ,4,4)
  
}

# Make the rest riverine
lakeStations <- filter(RegionalResults4, RorL == 'L') %>%
  select(-RorL)

riverStations <- filter(RegionalResults4, RorL == 'R') %>%
  select(-c(RorL))

# Make sure we have everyone
nrow(lakeStations) + nrow(riverStations) == nrow(RegionalResults4)
...
write.csv(select(lakeStations,-rowname), paste('processedStationData/final2020data/RegionalResultsLake_',
                                  regionCode,'CitMonNonAgencyFINAL.csv',sep=''), row.names=FALSE)
saveRDS(RegionalResults4,'processedStationData/final2020data/RegionalResults4.RDS')
