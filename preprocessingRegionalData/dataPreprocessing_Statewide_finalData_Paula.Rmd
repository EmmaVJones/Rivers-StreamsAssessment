...
# For BRRO we are dealing with all riverine stations separately so we do not accidentally duplicate AU information
...
previousCycleStationTable <- stationTableLake %>%
  group_by(STATION_ID) %>%
  mutate(extra= n()) %>%
...
rm(conventionals_D_Region_NoEstuary);rm(cleo) # clean up workspace
```
...
So now we have `r nrow(conventionals_D_Region_AU)` 

U
```{r}
y <- filter(conventionals_D_Region, !(FDT_STA_ID %in% previousCycleStationTable$STATION_ID)) %>%
  filter()
```
