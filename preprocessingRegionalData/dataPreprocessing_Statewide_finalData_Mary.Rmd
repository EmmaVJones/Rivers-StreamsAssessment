...
leaflet(conventionals_D_Region) %>% 
  ...
  addPolygons(data=assessmentLayerSelection,color='black',fillColor='blue', fillOpacity = 0.5,weight=2, stroke=T,
              group="Selected Assessment Region",
                  popup=popupTable(assessmentLayerSelection, zcol=c("HUC12","VAHU6","ASSESS_REG","Location",
                                                                 "VaName","PC_Water","Tidal","VAHU5"  ))) %>%
  addPolygons(data=assessmentLayer,color='navy',fill=0.9,stroke=0.1,group="Statewide Assessment Layer",
                  popup=popupTable(assessmentLayer, zcol=c("HUC12","VAHU6","ASSESS_REG","Location",
                                                                 "VaName","PC_Water","Tidal","VAHU5"  ))) %>% 
  hideGroup('Statewide Assessment Layer') %>%
  addCircleMarkers(data=conventionals_D_Region,color='yellow', fillColor='blue', radius = 5,
                   fillOpacity = 0.5,opacity=1,weight = 2,stroke=T,group="conventionals_D_Region",
                   popup=popupTable(conventionals_D_Region, zcol=c("FDT_STA_ID","STA_DESC","Deq_Region","STA_REC_CODE",
                                                  "FDT_SSC_CODE","FDT_SPG_CODE","Huc6_Huc_8","Huc6_Huc_8_Name",
                                                  "Huc6_Name","Huc6_Vahu5","Huc6_Huc_12","Huc6_Huc_12_Name","Huc6_Vahu6",
                                                  "STA_LV1_CODE","LV1_DESCRIPTION","STA_LV2_CODE","LV2_DESCRIPTION",
                                                  "STA_LV3_CODE","LV3_DESCRIPTION","STA_CBP_NAME","Basin"))) %>%
  addCircleMarkers(data=conventionals_D,color='red', fillColor='red', radius = 4,
                   fillOpacity = 0.5,opacity=1,weight = 0,stroke=T,group="conventionals_D",
                   popup=popupTable(conventionals_D_Region, zcol=c("FDT_STA_ID","STA_DESC","Deq_Region","STA_REC_CODE",
                                                  "FDT_SSC_CODE","FDT_SPG_CODE","Huc6_Huc_8","Huc6_Huc_8_Name",
                                                  "Huc6_Name","Huc6_Vahu5","Huc6_Huc_12","Huc6_Huc_12_Name","Huc6_Vahu6",
                                                  "STA_LV1_CODE","LV1_DESCRIPTION","STA_LV2_CODE","LV2_DESCRIPTION",
  ...
  addLayersControl(baseGroups=c('Nat Geo World Map','Esri World Imagery','Open Street Map'),
                       overlayGroups = c('conventionals_D_Region','conventionals_D',
                                         'Selected Assessment Region','Statewide Assessment Layer'),
...
After you have completed adding all FDT_STA_ID's to the filter statement, press the 'Play' button on the top right of the chunk and the code will be executed for you. The desired stations will be added back to the conventionals_D_Region object for further processing steps.

```{r conventionals_D_Region adjustments}

conventionals_D_add <- filter(conventionals_D, FDT_STA_ID %in% c('')) # '9-NEW107.50','9-NEW107.51','9-WFC017.31' were noted in an email exchange
...
# From Mary (Rivers)
stationTable <- read_excel('data/BRRO/STREAMS_ir_mon_stations_2Apr2019.xlsx', sheet='2018IR_Stations_FINAL') %>%
  rename('COMMENTS'='Notes' )
stationTable$WAT_MET_VIO <- as.numeric(stationTable$WAT_MET_VIO)
stationTable$FISH_MET_VIO <- as.numeric(stationTable$FISH_MET_VIO)

#From Paula (lakes)
stationTableLake <- read_excel('data/BRRO/Paula_tbl_ir_mon_stations_BRRO_2018_JRH_v1 (2).xlsx', sheet ='tbl_ir_mon_stations_2018_LAKES') %>%
  dplyr::select(ID305B_1:COMMENTS) %>%
  filter(!is.na(ID305B_1)) # some extra rows got in there
stationTableLake$SED_MET_VIO <- as.numeric(stationTableLake$SED_MET_VIO)

# Now combine and get rid of duplicate entries
previousCycleStationTable <- bind_rows(stationTable,stationTableLake) %>%
  #distinct(stationTable, STATION_ID, ID305B_1, ID305B_2, ID305B_3, .keep_all = T) %>%
  group_by(STATION_ID) %>%
  mutate(extra= n()) %>%
  select(extra, everything())

z <- filter(previousCycleStationTable, extra > 1) # keep a list of the problem sites

# Reorganize columns to match Cleo's system
cleo <- read_excel('data/tbl_ir_mon_stations_template.xlsx')

previousCycleStationTable <- dplyr::select(previousCycleStationTable, names(cleo))
...
# add back in duplicate issues where lacustrine and riverine
...
RegionalResultsCombinedAll <-read_csv('data/BRRO/RegionalResultsCombined_BRROFINAL.csv')

# Quick duplicate check
dupes <- RegionalResultsCombinedAll$FDT_STA_ID [duplicated(RegionalResultsCombinedAll$FDT_STA_ID )] 


View(filter(RegionalResultsCombinedAll, FDT_STA_ID %in% dupes) %>% arrange(FDT_STA_ID))


# 4ABMA002.00" "4ABWR019.75" "9-PKC004.65" "4ALVL001.11" are all cool because they have one entry with L in STATION_TYPE
# designation and one that is still riverine. The ID305B column doesnt reflect this



# 4APGG003.29 in there 4 times, doubled for lake and stream, everything else is okay to be duplicated
RegionalResultsCombinedAll2 <- RegionalResultsCombinedAll %>% distinct() # gets rid of duplicated PGG003.29 rows

View(filter(RegionalResultsCombinedAll2, FDT_STA_ID %in% dupes) %>% arrange(FDT_STA_ID))
...
Now time to fix manual WQS assignments.

```{r WQS assignments}

noWQS <- read_csv('data/BRRO/RegionalResultsCombined_NoWQS_BRRO.csv')

fixWQS <- filter(RegionalResultsCombinedAll3, FDT_STA_ID %in% noWQS$FDT_STA_ID)
fineWQS <- filter(RegionalResultsCombinedAll3, !(FDT_STA_ID %in% fixWQS$FDT_STA_ID))

# Cant fix riverine all at once because the individual  basin layer WQS is different than the whole state layer WQS
# Mary assigned OBJECTID based on basin layer WQS so need to find each basin, join to each layer by OBJECTID, 
# and repeat for next basin

check <- fixWQS %>% st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
                 remove = F, # don't remove these lat/lon cols from df
                 crs = 4326) # add coordinate reference system, needs to be geographic for now bc entering lat/lng,
st_write(check,'data/BRRO/fixWQS.shp')
```
...
james <-  sf::st_read(dsn = "C:/HardDriveBackup/R/GitHub/Rivers-StreamsAssessment/GIS/updatedWQS/James_071218.gdb", 
                      layer = "riverine") 
james$OBJECTID <- as.numeric(row.names(james))

jamesSites <- filter(fixWQS, Basin == 'James River Basin') %>%
  select(-c(GNIS_ID:Shape_Leng)) %>%
  left_join(james, by='OBJECTID') %>%
  rename(WQS_COMMEN= 'WQS_COMMENT', SECTION_DE= 'SECTION_DESCRIPTION', Shape_Leng = 'Shape_Length') %>%
  select(names(fixWQS))
rm(james)
```
...
Roanoke River Basin
```{r James WQS manual}
roa <-  sf::st_read(dsn = "C:/HardDriveBackup/R/GitHub/Rivers-StreamsAssessment/GIS/updatedWQS/RoanokeYadkin_07262018.gdb", 
                      layer = "riverine") 
roa$OBJECTID <- as.numeric(row.names(roa))

roaSites <- filter(fixWQS, Basin == 'Roanoke River Basin') 

# river
roaR <- filter(roaSites, lakeStation == 'river') %>%
  select(-c(GNIS_ID:Shape_Leng)) %>%
  left_join(roa, by='OBJECTID') %>%
  rename(WQS_COMMEN= 'WQS_COMMENT', SECTION_DE= 'SECTION_DESCRIPTION', Shape_Leng = 'Shape_Length') %>%
  select(names(fixWQS))
rm(roa)
...
roaLake <- filter(roaSites, lakeStation== 'lake') %>%
  select(-c(GNIS_ID:Shape_Leng)) %>%
  left_join(roa, by='OBJECTID') %>%
  rename( SECTION_DE= 'SECTION_DESCRIPTION', Shape_Leng = 'Shape_Length') %>%
  mutate(WQS_COMMEN= NA, GNIS_ID= NA,GNIS_Name= NA,FType= NA,FCode= NA,BASIN= NA,
         Edit_Date= NA, StreamType= NA, Backlog= NA) %>%
  select(names(fixWQS))
rm(roa)
```
...
new <-  sf::st_read(dsn = "C:/HardDriveBackup/R/GitHub/Rivers-StreamsAssessment/GIS/updatedWQS/New_07262018.gdb", 
                      layer = "riverine") 
new$OBJECTID <- as.numeric(row.names(new))

newSites <- filter(fixWQS, Basin == 'New River Basin') %>%
  select(-c(GNIS_ID:Shape_Leng)) %>%
  left_join(new, by='OBJECTID') %>%
  rename(WQS_COMMEN= 'WQS_COMMENT', SECTION_DE= 'SECTION_DESCRIPTION', Shape_Leng = 'Shape_Length') %>%
  select(names(fixWQS))
rm(new)
```
...
Make sure we didnt lose anyone and smash all together

```{r}
nrow(jamesSites) + nrow(roaR) + nrow(roaLake) + nrow(newSites) == nrow(fixWQS)

fixedWQS <- bind_rows(jamesSites, roaR, roaLake, newSites)

# Add back to whole 
RegionalResultsCombinedAll4 <- rbind(fineWQS, fixedWQS)

```



```{r old way before knew OBJECTID all messed up}
#fixWQS_R <- filter(fixWQS, lakeStation == 'river') %>%
#  dplyr::select(-c(GNIS_ID:Shape_Leng)) %>% # drop columns that I will repopulate from spatial layer
#  left_join(riverine, by= "OBJECTID") %>%
#  dplyr::select(names(RegionalResultsCombinedAll3))
...
#fixWQS_L <- filter(fixWQS, lakeStation == 'lake') %>%
#  dplyr::select(-c(GNIS_ID:Shape_Leng)) %>% # drop columns that I will repopulate from spatial layer
#  left_join(reservoirWQS, by= "OBJECTID") %>%
#  mutate(StreamType=NA) %>%
#  dplyr::select(names(RegionalResultsCombinedAll3)) 
#rm(reservoirWQS)


#names(fixWQS_R) == names(fixWQS_L)

#fixedWQS <- rbind(fixWQS_R, fixWQS_L)
...
Now split river and lake sites by AU designation. 


```{r split up freshwater}
lakeStations <- filter(RegionalResultsCombinedAll4, lakeStation=='lake') %>%
  select(-c(lakeStation))

# Make the rest riverine
riverStations <- filter(RegionalResultsCombinedAll4, lakeStation != 'lake') %>%
  select(-c(lakeStation))

# Make sure we didnt lose anyone
nrow(riverStations) + nrow(lakeStations) == nrow(RegionalResultsCombinedAll4)
...
```{r}
rm(roaSites); rm(roaR); rm(roaLake); rm(riverStations); rm(noWQS); rm(newSites); rm(lakeStations); rm(fixWQS); rm(fixedWQS); rm(jamesSites);rm(fineWQS)
```
...
# This is the station table you have edited including all riverine and lake stations with all necessary AU and WQS information fixed for your region.
#RegionalResultsCombinedAll <- read_csv('processedStationData/final2020data/RegionalResultsCombined_final.csv')
RegionalResultsCombinedAll <- read_csv('processedStationData/final2020data/MaryFiles/RegionalResultsCombined_BRROFINAL.csv')
...
# 4APGG003.29 in there 4 times, doubled for lake and stream, everything else is okay to be duplicated
RegionalResultsCombinedAll2 <- RegionalResultsCombinedAll %>% distinct() %>% # gets rid of duplicated PGG003.29 rows
  mutate(keep=ifelse(FDT_STA_ID != '4ALVL001.11', 1, ifelse(FDT_STA_ID == '4ALVL001.11' & STATION_TYPE_2 == 'L', 1, 0))) %>%
  filter(keep == '1') %>%
  select(-keep)
  
#filter(!(FDT_STA_ID == '4ALVL001.11' & STATION_TYPE_2 == 'L'))

# Find all stations that have lake designations
RegionalResultsCombinedAll3 <- mutate(RegionalResultsCombinedAll2, lakeStation= ifelse(grepl("L",STATION_TYPE_1) | grepl("L",STATION_TYPE_2) |
                               grepl("L",STATION_TYPE_3), 'lake','river'))
