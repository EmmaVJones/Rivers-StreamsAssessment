...
RegionalResultsCombinedAll <-read_csv('data/PRO/RegionalResultsCombined_PRO.csv')
...
Now double check that no estuarine AUs are secretly in the dataset.

```{r}
# lazy
RegionalResultsCombinedAll3$EorR <- NA
for(i in 1: nrow(RegionalResultsCombinedAll3)){
  RegionalResultsCombinedAll3$EorR[i] <- substr(strsplit(as.character(RegionalResultsCombinedAll3$ID305B_1[i]), '-')[[1]][2] ,4,4)

}


estuaryDoubleCheck <- filter(RegionalResultsCombinedAll3, EorR == 'E' )

justFreshwater <- filter(RegionalResultsCombinedAll3, !(FDT_STA_ID %in% estuaryDoubleCheck$FDT_STA_ID))

# make sure we aren't losing anyone
nrow(estuaryDoubleCheck) + nrow(justFreshwater) == nrow(RegionalResultsCombinedAll3)

```


If there are, we will give those back to the assessor in a separate dataset. Now split river and lake sites by AU designation. 


```{r split up freshwater}
lakeStations <- filter(justFreshwater, lakeStation=='lake') %>%
  select(-c(lakeStation, EorR))

# Make the rest riverine
riverStations <- filter(justFreshwater, lakeStation != 'lake') %>%
  select(-c(lakeStation, EorR))

# Make sure we didnt lose anyone
nrow(riverStations) + nrow(lakeStations) == nrow(justFreshwater)

```


Need to fix lake duplicates.

```{r lake dupes}
lakeWQS <- st_read('GIS/updatedWQS/reservoir.shp')%>%
  st_transform( st_crs(4326)) # transform to WQS84 for spatial intersection

z1 <- filter(lakeWQS, OBJECTID %in% c(15422))
z2 <- filter(lakeWQS, OBJECTID %in% c(15433))
mapview(z1)+mapview(z2)
# so 15422 looks like just the river polygon, 15433 is the actual lake

lakeStations <- filter(lakeStations, !(OBJECTID == 	15422))
rm(z1);rm(z2);rm(lakeWQS)
```



Need to make sure all riverine sites have WQS. If they don't at this stage that means they are probably tidal fresh and need to be attached to estuary WQS information.

Dont run for PRO bc none in estuarine

```{r WQs for tidal fresh,eval=F}
tidalFresh <- filter(riverStations, is.na(OBJECTID)) %>% # get just those riverine sites that didnt join to class III-VI WQS layer
  select(-c(OBJECTID:Shape_Leng)) %>% # remove columns we aim to add
  st_as_sf(coords = c("Longitude", "Latitude"),  # make spatial layer using these columns
           remove = F, # don't remove these lat/lon cols from df
           crs = 4326) # add coordinate reference system, needs to be geographic for now bc entering lat/lng,
...
# find lake AUs, if Any
tidalFresh2 <- st_join(tidalFresh, estuaryPolyWQS, join = st_intersects) %>%
  select(-c(OBJECTID_1)) # remove bonus occurrence of this field

tidalFreshFixed <- filter(tidalFresh2, !is.na(OBJECTID))  # only keep rows where join successful

tidalFreshLines <- filter(tidalFresh2, is.na(OBJECTID)) %>% # only keep rows where NOT join successful
  select(-c(Point.Unique.Identifier:Shape_Area))# remove columns we aim to add

# make shapefile
st_write(tidalFreshLines,
         paste('processedStationData/final2020data/',
               regionCode,'tidalFreshHelp.shp',sep=''))



rm(estuaryPolyWQS) # clean up workspace



# now do segment snapping analysis

estuaryLinesWQS <-  st_read('C:/HardDriveBackup/R/GitHub/Rivers-StreamsAssessment/GIS/updatedWQS/estuaryLinesWQS.shp') %>%
  st_transform(crs = 102003)  # transform to Albers for spatial intersection

tidalFreshLines2 <- snapAndOrganizeWQSestuary(tidalFreshLines, estuaryLinesWQS,  bufferDistances = seq(10,80,by=10))

# fix before merging
tidalFreshFixed <- tidalFreshFixed %>% st_set_geometry(NULL) %>%
  mutate(Trout=NA, StreamType=NA) %>%
    select(names(riverStations))
tidalFreshLines3 <- rename(tidalFreshLines2, 
                           "Point.Unique.Identifier" = `Point Unique Identifier`,
                           "Buffer.Distance" = `Buffer Distance`) %>%
  select(names(riverStations))

names(tidalFreshLines3) ==  names(tidalFreshFixed) == names(riverStations)


riverStations2 <- filter(riverStations, !(FDT_STA_ID %in% tidalFreshLines3$FDT_STA_ID)) %>%
  filter(!(FDT_STA_ID %in% tidalFreshFixed$FDT_STA_ID))


#riverStationsAll <- rbind(riverStations2, tidalFreshFixed, tidalFreshLines3)
riverStationsAll <- riverStations

nrow(riverStationsAll) + nrow(lakeStations) == nrow(justFreshwater)


# Fix WQS error by hand
#View(filter(riverStationsAll, FDT_STA_ID == '5BADA002.34')) # no wqs because it was left off updated chowan wqs riverine layer

#riverStationsAll <- mutate(riverStationsAll, 
#                            BASIN = ifelse(FDT_STA_ID == '5BADA002.34', '5B', BASIN),
#                            WATER_NAME = ifelse(FDT_STA_ID == '5BADA002.34', 'Unnamed tributary', WATER_NAME),
#                            SEC = ifelse(FDT_STA_ID == '5BADA002.34', '3', SEC),
#                            CLASS = ifelse(FDT_STA_ID == '5BADA002.34', 'III', CLASS),
#                            SECTION_DE = ifelse(FDT_STA_ID == '5BADA002.34', 
#                                                'Lake Drummond, including feeder ditches, and all interstate tributaries of the Dismal Swamp between Virginia and North Carolina.', SECTION_DE),
#                            Basin_Code = ifelse(FDT_STA_ID == '5BADA002.34', 'Chowan-Albermarle', Basin_Code))
...
write.csv(riverStationsAll, paste('processedStationData/final2020data/RegionalResultsRiverine_',
                                         regionCode,'FINAL.csv',sep=''), row.names=FALSE)
