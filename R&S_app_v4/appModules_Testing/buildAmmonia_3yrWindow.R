...
stationTable <- read_csv('C:/HardDriveBackup/R/GitHub/Rivers-StreamsAssessment/R&S_app_v2/processedStationData/draft2020data/RegionalResultsRiverine_BRRO_NOMISSINGDATA.csv')
...
AUData <- #filter(conventionals_HUC, ID305B_1 %in% 'VAW-H01R_JMS04A00' | 
          ...
          #         ID305B_2 %in% 'VAW-H01R_JMS04A00')%>% 
  left_join(conventionals_HUC,WQSvalues, by = 'CLASS')

x <-filter(AUData, FDT_STA_ID %in% '2-HUO005.87') 
x <-filter(AUData, FDT_STA_ID %in% '2-JKS028.69') 
