...
stationTable <- 
  # PRO
  #read_csv('processedStationData/RegionalResultsRiverine_PROFINAL.csv') %>%
...
for(i in 1:nrow(stationTable)){
  ...
  if(all(stationData$STA_LV3_CODE %in% c("CMON", "NONA", "NONA "))){
      ...
      citMonStationData <- filter(citMon, FDT_STA_ID %in% stationTable$FDT_STA_ID[i])
      ...
      nonAStationData <- filter(nonA, FDT_STA_ID %in% stationTable$FDT_STA_ID[i])
...
Save results.
```{r saveResults}
regionCode <- 'SWRO'#'TRO'#'PRO'#'BRRO'#'SWRO'#'TRO'#
...
Optional section:
Compare previous results to results just run with compareDF to highlight results differences

```{r}
#oldresults <- read.csv('stationTableResultsBRRO2019-05-21.csv',na.strings=c("NA","NaN", " ")) %>%
#oldresults <- read.csv('stationTableResultsPRO2019-09-30.csv',na.strings=c("NA","NaN", " ")) %>%
#oldresults <- read.csv('stationTableResultsTRO2019-05-03.csv',na.strings=c("NA","NaN", " ")) %>%
oldresults <- read.csv('stationTableResultsSWRO2019-05-03.csv',na.strings=c("NA","NaN", " ")) %>%
  select(STATION_ID, TEMP_VIO:SED_MET_STAT, BENTHIC_STAT:NUT_CHLA_STAT)
library(compareDF)


comparisonTable <- compare_df(select(stationTableResultsWithComments,STATION_ID, TEMP_VIO:SED_MET_STAT, BENTHIC_STAT:NUT_CHLA_STAT),
                              oldresults, 'STATION_ID')

print(comparisonTable$html_output) #easier output to read

View(comparisonTable$change_count) # sum of changes

comparisonTable$change_summary
```


```{r}

joincol <- select(comparisonTable$comparison_df,STATION_ID) %>% rename('STATIONID'='STATION_ID')
easierReading <- cbind(joincol, comparisonTable$comparison_table_diff) 
```


find places with different Temp violation counts:
```{r}
filter(easierReading, TEMP_VIO == '+')$STATIONID
filter(easierReading, DO_VIO == '+')$STATIONID
```
