...
#x <-filter(conventionals_HUC, FDT_STA_ID %in% '2-JKS018.68')# '4APKP-4-DRBA') '8-YRK022.70')#


#parameter <- "FDT_FIELD_PH"
#parameterRemark <- "FDT_FIELD_PH_RMK"
...
citmonParameterCount <- function(x, parameter, parameterRemark){
  if(unique(x$STA_LV3_CODE) %in% c("CMON", "NONA", "NONA ")){
    ...
    if(nrow(z) > 0){
      return(paste(parameter,': ', nrow(z), ' samples at Level II', sep=''))
    } else {
      return(NULL)
    }
  } else {
...
#citmonParameterCount(x, "DO", "DO_RMK")#"FDT_FIELD_PH", "FDT_FIELD_PH_RMK")
